...
class mysqlCon:
    ...
    def fndbconn():
        ...
        conn = mysqlConnector.connect(host='localhost',user='root',password='sagar@123',database='testdb')
        if conn:
            return 1
        else:
            return 0
    ...
    def fnRegister(username,password,tmpemailid):
        ...
        mycursor.execute(f"insert into users(username,password,email_id) values('{username}','{password}','{tmpemailid}')")
        conn.commit()
        conn.close()